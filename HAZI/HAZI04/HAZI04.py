...
def math_passed_count(df_data) -> int:
    new_df = df_data.copy()
    subset = new_df[new_df['math score'] >= 50 ]
    shape = np.shape(subset)
    return shape[0]
...
def average_scores(df_data) -> pd.core.frame.DataFrame:
    new_df = df_data.copy()
    group = new_df.groupby('parental level of education')['math score', 'reading score', 'writing score'].mean()
    return group
...
def add_age(df_data) -> pd.core.frame.DataFrame:
    new_df = df_data.copy()
    np.random.seed(42)
    age = np.random.randint(18, 67, new_df.shape[0])
    new_df['age'] = age
    return new_df
...
def female_top_score(df_data) -> tuple:
    new_df = df_data.copy()
    females = new_df.loc[np.where(new_df['gender'] == 'female')]
    best_female = females['math score'].max(), females['reading score'].max(), females['writing score'].max()
    return best_female
...
def add_grade(df_data) -> pd.core.frame.DataFrame:
    new_df = df_data.copy()
    grade = (new_df['math score'] + new_df['reading score'] + new_df['writing score']) / 3
